...
export default function Dashboard() {
  ...
  const getRecentActivity = () => {
    ...
    invoices.slice(-5).forEach(invoice => {
      activities.push({
        ...
        time: new Date(invoice.date).toLocaleString()
      });
    });
  ...
  const getTotalItemsSold = () => {
    return invoices.reduce((total, invoice) => 
      total + invoice.items.reduce((itemTotal, item) => itemTotal + item.quantity, 0), 0
    );
  };
          ...
          React.createElement('form', { 
            ...
            React.createElement('div', {},
              ...
              React.createElement('textarea', { 
                ...
                onChange: (e: any) => setConfigForm(prev => ({...prev, address: e.target.value})),
                className: 'mt-1 block w-full border border-gray-300 rounded-md px-3 py-2',
                rows: 3
              })
            ),
            React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-2 gap-4' },
              React.createElement('div', {},
                ...
                React.createElement('input', { 
                  ...
                  onChange: (e: any) => setConfigForm(prev => ({...prev, phone: e.target.value})),
                  className: 'mt-1 block w-full border border-gray-300 rounded-md px-3 py-2'
                })
              ),
              React.createElement('div', {},
                ...
                React.createElement('input', { 
                  ...
                  onChange: (e: any) => setConfigForm(prev => ({...prev, email: e.target.value})),
                  className: 'mt-1 block w-full border border-gray-300 rounded-md px-3 py-2'
                })
              )
            ),
        ...
        React.createElement('div', { className: 'bg-white p-6 rounded-lg shadow' },
          ...
          React.createElement('div', { className: 'space-y-4' },
            ...
            React.createElement('div', { className: 'flex items-center' },
              React.createElement('input', { 
                type: 'checkbox',
                checked: configForm.lowStockAlerts || false,
                onChange: (e: any) => setConfigForm(prev => ({...prev, lowStockAlerts: e.target.checked})),
                className: 'mr-2'
              }),
              React.createElement('label', { className: 'text-sm text-gray-700' }, 'Alertas de stock bajo')
            )
          )
        )
      )
